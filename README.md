...
- Замените файл `main.csv` своим набором данных. Убедитесь, что в CSV-файле есть такие столбцы, как Табельный номер, ФИО исполнителя, Ангар, Дата, Номер заказа, Заказчик, ИД номер заказчика, Тип услуги, Марка автомобиля, Модель автомобиля, Тип работ, Стоимость деталей (руб), Стоимость работы (руб), Общая стоимость (руб).
